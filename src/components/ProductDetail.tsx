...
import { 
  MapPin, Phone, Building2, ChevronLeft, ArrowRight, Share2, 
  Heart, CheckCircle2, Shield, Compass, Tag, Layers, Bookmark, Sparkles, MessageCircle, Bath, Facebook, Link as LinkIcon
} from 'lucide-react';
...
const MapViewer = React.memo(({ mapHtml, address }: { mapHtml?: string, address?: string }) => {
  if (mapHtml && (mapHtml.startsWith('<iframe') || mapHtml.includes('google.com/maps'))) {
    return (
      <div 
        className="w-full aspect-[21/9] rounded-lg overflow-hidden border border-slate-800 shadow-inner bg-slate-950 [&_iframe]:w-full [&_iframe]:h-full [&_iframe]:border-0"
        dangerouslySetInnerHTML={{ __html: mapHtml.includes('iframe') ? mapHtml : `<iframe src="${mapHtml}" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>` }}
      />
    );
  }
  ...
  return (
    <div className="w-full aspect-[21/9] rounded-lg overflow-hidden border border-slate-800 shadow-inner bg-slate-950">
      <iframe
        width="100%"
        height="100%"
        style={{ border: 0 }}
...
export default function ProductDetail({ productId, onNavigate, onShowNotification, logoUrl }: ProductDetailProps) {
    ...
    async function loadProductData() {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          onShowNotification('Sản phẩm không có hoặc đã bị gỡ xuống khỏi hệ thống.', 'error');
        ...
        historyList.sort((a, b) => updatedList.indexOf(a.id) - updatedList.indexOf(b.id));
  ...
  const handleInquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setClientPhone('');
      setClientDemand('');
      onShowNotification(`Đã gửi yêu cầu tư vấn thành công! Nhân viên sẽ gọi lại cho quý khách.`, 'success');
    } catch (err) {
      console.error(err);
      onShowNotification('Đã xảy ra sự cố gửi yêu cầu. Vui lòng quay lại sau.', 'error');
    } finally {
  ...
  if (loading) {
    return (
      <div className="py-44 text-center space-y-4 max-w-sm mx-auto" id="product-detail-loader">
        <div className="w-12 h-12 border-4 border-amber-500 border-t-transparent rounded-full animate-spin mx-auto" />
        <p className="text-slate-400 text-xs font-light font-mono">Đang tải chi tiết...</p>
      </div>
    );
  }
  ...
  products.forEach(p => {
    ...
    const matchedConfigCat = productCategoriesExt.find(c => c.name.trim().toLowerCase() === cat.toLowerCase());
  ...
  const relatedCategoryProducts = products.filter(p => p.category && product.category && p.id !== product.id && p.category.trim().toLowerCase() === product.category.trim().toLowerCase());

  // Categorized Sliders at the footer
  const footerLatestSales = products.filter(p => p.type !== 'rent').slice(0, 8);
  const footerLatestRents = products.filter(p => p.type === 'rent').slice(0, 8);

  const productImages = product.imageUrls && product.imageUrls.length > 0
    ? product.imageUrls
    : [product.imageUrl || 'https://placehold.co/1200x600/1e293b/a4b5fd?text=No+Image'];

  const rawBaseRating = product.baseRating || 5;
  const rawBaseCount = product.baseReviewCount || 0;
  const computedTotalStars = rawBaseRating * rawBaseCount + (product.userTotalRating || 0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-[20px] pt-[15px] pb-[10px] space-y-16 animate-in fade-in" id={`product-detail-viewport-${product.id}`}>
      <Helmet>
      ...
      <nav className="flex items-center justify-between text-xs text-slate-400 border-b border-slate-900 pb-[5px] mb-[15px]" id="detail-breadcrumb">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span>/</span>
          <button 
            onClick={() => onNavigate({ screen: product.type === 'rent' ? 'latest-rents' : 'latest-sales' })}
            className="hover:text-amber-500 cursor-pointer"
          >
            {product.type === 'rent' ? 'Cho thuê' : 'Chuyển nhượng'}
          </button>
          <span>/</span>
          <span className="text-amber-500 max-w-[120px] sm:max-w-none truncate">{product.category}</span>
          <span>/</span>
          <span className="text-slate-200 font-semibold max-w-[150px] sm:max-w-none truncate">{product.title}</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-6 space-y-8" id="detail-pane-left">
          ...
          <div className="space-y-4 !mb-[10px]">
            <div className="relative aspect-[16/10] overflow-hidden rounded-lg border border-slate-900 bg-slate-950 shadow-xl">
              ...
              <div className="absolute top-4 left-4">
                <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow ${
                  product.type === 'rent' ? 'bg-[#00b894] text-white' : 'bg-[#ff4d4f] text-white'
                }`}>
            ...
            <div className="flex gap-2 overflow-x-auto pb-[5px] scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
              {(product.imageUrls && product.imageUrls.length > 0 ? product.imageUrls : [product.imageUrl || sampleThumbs[0], ...sampleThumbs.slice(1)]).map((imgUrl, thumbIdx) => (
                <button
                  key={thumbIdx}
                  ...
                  className={`w-16 h-12 lg:w-20 lg:h-16 shrink-0 rounded-lg overflow-hidden border transition-all cursor-pointer bg-slate-950 ${
                    selectedImage === imgUrl ? 'border-amber-500 ring-1 ring-amber-500/30' : 'border-slate-850 hover:border-slate-700'
                  }`}
                >
                  <img loading="lazy" decoding="async" src={(imgUrl) || undefined} alt={`${product.title} - ảnh thu nhỏ ${thumbIdx + 1}`} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-slate-900 pt-[10px] pb-4 px-[10px] sm:px-[15px] !mb-[5px] rounded-lg border border-slate-900 text-left">
            ...
            <div className="grid grid-cols-3 gap-4 !mt-[10px] pb-0 border-b border-slate-850/60">
              <div className="space-y-0.5 h-12">
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Giá bán</span>
                <p className="font-display font-extrabold text-base text-amber-500 truncate">{product.priceText}</p>
              </div>

              <div className="space-y-0.5">
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Diện tích</span>
                <p className="font-display font-semibold text-[13px] md:text-sm text-slate-200">{product.area || 120} m²</p>
              </div>

              <div className="space-y-0.5">
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Liên hệ</span>
                <p className="font-display font-semibold text-[13px] md:text-sm text-slate-200">
                  <a href="tel:0932966700" className="hover:text-amber-500 transition-colors">0932 966 700</a>
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-3 sm:gap-x-4 text-[13px] !pt-[5px]">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                    ...
                    <span className="text-[11px] sm:text-[13px]">Giá bán</span>
                  </div>
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.priceText || "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                    <Layers className="w-4 h-4 text-amber-500" />
                    <span className="text-[11px] sm:text-[13px]">Diện tích</span>
                  </div>
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.area ? `${product.area} m²` : "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                    <Bookmark className="w-4 h-4 text-amber-500" />
                    <span className="text-[11px] sm:text-[13px]">Phòng ngủ</span>
                  </div>
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.bedrooms ? `${product.bedrooms} PN` : "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                    <Bath className="w-4 h-4 text-amber-500" />
                    <span className="text-[11px] sm:text-[13px]">Phòng vệ sinh</span>
                  </div>
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.toilets ? `${product.toilets} WC` : "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                  ...
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.floors || "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                    <Compass className="w-4 h-4 text-amber-500" />
                    <span className="text-[11px] sm:text-[13px]">Hướng nhà</span>
                  </div>
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.direction || "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                  ...
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{(product as any).frontage ? `${(product as any).frontage}m` : "Đang cập nhật"}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-start sm:gap-6 border-b border-slate-800/30 pb-1 sm:pb-[2px]">
                  <div className="flex items-center gap-1.5 text-slate-400 w-full sm:w-24 shrink-0 mb-1 sm:mb-0">
                  ...
                  <span className="text-slate-200 font-semibold text-left text-xs sm:text-[13px] line-clamp-1">{product.legalStatus || "Đang cập nhật"}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 text-left">
            <div className="flex border-b border-slate-900 pb-px gap-1 !mb-[10px] !pb-0 h-[35px] !pt-[5px]">
              <button
                ...
                className={`px-5 py-3 text-sm font-semibold tracking-wider relative cursor-pointer !py-0 !px-[15px] h-[30px] ${
                  activeTab === 'desc' ? 'text-amber-500 font-bold' : 'text-slate-500 hover:text-white'
                }`}
              >
                Thông tin chi tiết
                {activeTab === 'desc' && (
                  <div className="absolute bottom-0 inset-x-0 h-0.5 bg-amber-500" />
                )}
              </button>

              <button
                onClick={() => setActiveTab('map')}
                className={`px-5 py-3 text-sm font-semibold tracking-wider relative cursor-pointer ${
                  activeTab === 'map' ? 'text-amber-500 font-bold' : 'text-slate-500 hover:text-white'
                }`}
              >
                Vị trí
                {activeTab === 'map' && (
            ...
            <div className={`bg-slate-900 border border-slate-900 rounded-lg min-h-[160px] ${activeTab === 'map' ? 'p-[1px]' : '!py-[10px] !px-[15px]'}`}>
              <div style={{ display: activeTab === 'desc' ? 'block' : 'none' }}>
                {product.description ? (
                  ...
                  <p className="text-slate-500 text-xs italic">Không có tài liệu thuyết minh đi kèm.</p>
                )}
              </div>

              <div className="w-full" style={{ display: activeTab === 'map' ? 'block' : 'none' }}>
                <MapViewer mapHtml={product.mapHtml} address={`${product.title}, ${product.district}`} />
              </div>

              <div className="mt-[24px] px-[10px]">
          ...
          {/* "View more products in [category]" section */}
          {relatedCategoryProducts.length > 0 && (
            <div className="pt-2 text-left">
              <button
                type="button"
                onClick={() => onNavigate({ screen: 'category-product', categoryName: product.category })}
                className="bg-transparent border-none p-0 cursor-pointer group flex items-center"
              >
                <h3 className="font-display font-medium text-[15px] text-amber-500 group-hover:text-amber-400 group-hover:underline m-0">
                  Xem thêm {product.category} ({relatedCategoryProducts.length})
                </h3>
              </button>
            </div>
          )}
        ...
        {/* =========================================================
            ...
            ========================================================= */}
        <div className="lg:col-span-3 space-y-6 lg:sticky lg:top-[10px] self-start" id="detail-pane-right">
          ...
          <div className="bg-slate-900 border border-amber-500/40 p-6 rounded-lg space-y-3 relative text-left h-[432px]">
            ...
            <div className="text-center">
              <div className="flex flex-col items-center justify-center mb-3 gap-2">
                <img loading="lazy" decoding="async"
                  src={product.avatarUrl || "https://placehold.co/100x100/1e293b/c5a059?text=CV"} 
                  alt="Chuyên viên tư vấn" 
                  className="h-16 w-16 rounded-full object-cover border-2 border-amber-500/50" 
                  referrerPolicy="no-referrer"
                  onError={(e) => {
                    const target = e.target as HTMLImageElement;
                    target.src = 'https://placehold.co/100x100/1e293b/c5a059?text=CV';
                  }}
                />
            ...
            <div className="pt-4 flex flex-col gap-2">
              <a href="tel:0932966700" className="flex-1 bg-gradient-to-br from-[#0f9b0f] to-[#00b894] hover:brightness-110 shadow-[0_4px_15px_rgba(15,155,15,0.4)] py-2 px-2 rounded-full text-white font-bold transition-all flex items-center justify-center gap-1.5 border border-white/10 uppercase tracking-wide">
                <Phone className="w-3.5 h-3.5 drop-shadow-md fill-white" />
                <span className="text-[10px]">0932 966 700</span>
              </a>
              <a href="https://zalo.me/0932966700" target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-br from-[#0068ff] to-[#00a8ff] hover:brightness-110 shadow-[0_4px_15px_rgba(0,104,255,0.4)] py-2 px-2 rounded-full text-white font-bold transition-all flex items-center justify-center gap-1.5 border border-white/10 uppercase tracking-wide">
                <img loading="lazy" decoding="async" src="https://upload.wikimedia.org/wikipedia/commons/9/91/Icon_of_Zalo.svg" alt="Zalo" className="w-5 h-5 drop-shadow-md" />
        ...
        {/* =========================================================
            ...
            ========================================================= */}
        <div className="lg:col-span-3 space-y-6 lg:sticky lg:top-[10px] self-start" id="detail-pane-far-right">
            ...
            <div className="space-y-2">
              {Object.entries(categoryCounts).map(([catName, cnt]) => (
                <div
                  ...
                  className="flex justify-between items-center text-xs text-slate-300 hover:text-amber-500 cursor-pointer pt-[5px] pb-0 transition-colors border-b border-slate-950/40 last:border-0"
                >
          ...
          <div className="bg-slate-900 border border-slate-850 py-2.5 px-[15px] rounded-lg space-y-3 shadow-xl text-left">
            ...
            <div className="space-y-1.5 text-xs">
              <button onClick={() => onNavigate({ screen: 'san-pham', priceRange: 'under3' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Dưới 3 Tỷ
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', priceRange: '3to5' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Từ 3 Tỷ đến 5 Tỷ
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', priceRange: '5to10' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Từ 5 Tỷ đến 10 Tỷ
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', priceRange: '10to20' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Từ 10 Tỷ đến 20 Tỷ
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', priceRange: '20to50' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Từ 20 Tỷ đến 50 Tỷ
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', priceRange: 'over50' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Trên 50 Tỷ
              </button>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-850 py-2.5 px-[15px] rounded-lg space-y-3 shadow-xl text-left mb-5">
            ...
            <div className="space-y-1.5 text-xs">
              <button onClick={() => onNavigate({ screen: 'san-pham', areaRange: 'under100' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Dưới 100 m²
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', areaRange: '100to300' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Từ 100 m² đến 300 m²
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', areaRange: '300to500' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Từ 300 m² đến 500 m²
              </button>
              <button onClick={() => onNavigate({ screen: 'san-pham', areaRange: 'over500' })} className="block w-full text-left text-slate-400 hover:text-amber-500 py-1 cursor-pointer bg-transparent border-none">
                • Trên 500 m²
              </button>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-850 py-2.5 px-[15px] rounded-lg space-y-3 shadow-xl text-left">
            ...
            <div className="space-y-2">
              {Object.entries(districtCounts).length > 0 ? (
                ...
                <div className="text-slate-500 text-xs italic py-2">Chưa có dữ liệu khu vực</div>
              )}
            </div>
          </div>
        </div>
      ...
      <AdBanner slot="product-detail-deep" containerClassName="max-w-7xl mx-auto" />
      ...
      <section className="space-y-6 text-left" id="detail-bottom-sales">
        ...
        <div className="flex w-full justify-center border-t border-slate-800/50 pt-8 mt-4">
          <button onClick={() => onNavigate({ screen: 'latest-sales' })} className="bg-transparent border border-amber-500 text-amber-500 hover:bg-amber-500/10 text-xs font-semibold px-6 py-3 rounded-md cursor-pointer transition-colors text-[10px] !py-[5px] !px-[15px]">
            Xem tất cả BĐS Bán
          </button>
        </div>
      </section>
      ...
      <section className="space-y-6 text-left" id="detail-bottom-rents">
        ...
        <div className="flex w-full justify-center border-t border-slate-800/50 pt-8 mt-4">
          <button onClick={() => onNavigate({ screen: 'latest-rents' })} className="bg-transparent border border-amber-500 text-amber-500 hover:bg-amber-500/10 text-xs font-semibold px-6 py-3 rounded-md cursor-pointer transition-colors text-[10px] !pt-[5px] !pb-[6px] !px-[15px]">
      ...
      <section className="space-y-6 text-left pb-0" id="detail-bottom-projects">
        <div className="flex items-end justify-between pb-2 border-b border-white/10 mb-4">
          <h2 className="text-[15px] font-display font-medium text-white border-l-4 border-amber-500 pl-3 m-0">Dự án nổi bật</h2>
          <button
            type="button"
