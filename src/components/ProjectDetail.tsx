...
import { ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Image as ImageIcon, MapPin, Building2, Phone, Calendar, Compass, ShieldCheck, Heart, Share2, Sparkles, AlertCircle, CheckCircle2, FileText, LayoutGrid, Newspaper, Leaf, Banknote, HelpCircle, Facebook, Link as LinkIcon, MessageCircle } from 'lucide-react';
...
export default function ProjectDetail({ projectId, onNavigate, onShowNotification, logoUrl }: ProjectDetailProps) {
  const [project, setProject] = useState<Project | null>(null);
  const [relatedProjects, setRelatedProjects] = useState<Project[]>([]);
  const [relatedProducts, setRelatedProducts] = useState<any[]>([]);
  const [relatedNews, setRelatedNews] = useState<any[]>([]);
  const [targetNewsCategory, setTargetNewsCategory] = useState<string>('');
  const [targetProductCategory, setTargetProductCategory] = useState<string>('');
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'location' | 'amenity' | 'floor-plan' | 'price' | 'news' | 'contact'>('overview');
  ...
  const [currentFloorPlanImageIndex, setCurrentFloorPlanImageIndex] = useState(0);
  const [currentAmenityImageIndex, setCurrentAmenityImageIndex] = useState(0);
  const [currentFloorPlanTabId, setCurrentFloorPlanTabId] = useState<string | null>(null);
  const [showShareMenu, setShowShareMenu] = useState(false);
  const scrollDirection = useScrollDirection();

  const hasSubdivisionContent = useMemo(() => {
    if (!project) return false;
    const hasCards = project.subdivisionsCards && project.subdivisionsCards.some(c => c.name?.trim() || c.imageUrl?.trim());
    ...
    const hasImages = html.includes('<img') || html.includes('<iframe');
  ...
  const TABS = useMemo(() => {
    ...
    if (hasSubdivisionContent) {
      defaultTabs.push({ id: 'subdivision', label: 'Phân khu', icon: Building2 });
    }
    defaultTabs.push(
  ...
  const [isFloorPlanAutoplayPaused, setIsFloorPlanAutoplayPaused] = useState(false);
  const [isAmenityAutoplayPaused, setIsAmenityAutoplayPaused] = useState(false);

  // Gallery Autoplay
  useEffect(() => {
    const galleryCount = [project?.imageUrl, ...(project?.imageUrls || [])].filter(Boolean).length;
  ...
  // Floor Plan Autoplay
  useEffect(() => {
    if (!project?.floorPlanImages || project.floorPlanImages.length <= 1 || isFloorPlanAutoplayPaused) return;

    const intervalId = setInterval(() => {
      setCurrentFloorPlanImageIndex(prev => (prev === project.floorPlanImages!.length - 1 ? 0 : prev + 1));
  ...
  // Amenity Autoplay
  useEffect(() => {
    if (!project?.amenityImages || project.amenityImages.length <= 1 || isAmenityAutoplayPaused) return;
    
    const intervalId = setInterval(() => {
      setCurrentAmenityImageIndex(prev => (prev === project.amenityImages!.length - 1 ? 0 : prev + 1));
  ...
  useEffect(() => {
    async function loadProject() {
      try {
        ...
        if (docSnap.exists()) {
          const fetchedProject = { id: docSnap.id, ...docSnap.data() } as Project;
          ...
          if (docSnap.data().newsCategoryUrl) {
            ...
            try {
              ...
              console.error("Failed to parse categoryName from project newsCategoryUrl", e);
            }
          }
          ...
          newsSnap.forEach(doc => {
            const data = doc.data();
            if (data.approvalStatus !== 'rejected' && data.title?.trim() && data.description?.trim()) {
              if (targetCategory && targetCategory.length > 0) {
                // Filter by category exactly
                if (data.category && data.category.toLowerCase() === targetCategory.toLowerCase()) {
          ...
          if (docSnap.data().productCategoryUrl) {
            ...
            // Tries to parse categoryName if it looks like a URL
            try {
              const prodUrlMatch = targetProdCategory.match(/categoryName=([^&]+)/);
              if (prodUrlMatch && prodUrlMatch[1]) {
                targetProdCategory = decodeURIComponent(prodUrlMatch[1]).trim();
              }
            } catch (e) {
              console.error("Failed to parse categoryName from project productCategoryUrl", e);
            }
          }
          ...
          productsSnap.forEach((prodDoc) => {
            ...
            if (data.approvalStatus !== 'rejected' && data.name?.trim()) {
              if (targetProdCategory && targetProdCategory.length > 0) {
                // Filter by category exactly
                if (data.category && data.category.toLowerCase() === targetProdCategory.toLowerCase()) {
          ...
          onShowNotification("Dự án quy hoạch không tồn tại hoặc đã bị xóa.", "error");
  ...
  const handleConsultSubmit = async (e: React.FormEvent) => {
    ...
    if (!clientName.trim() || !clientPhone.trim()) {
      onShowNotification("Vui lòng điền họ tên và số điện thoại liên lạc.", "error");
      return;
    }
    ...
    try {
      ...
      setClientPhone('');
      setClientDemand('');
      onShowNotification("Đăng ký nhận lịch trình tham quan dự án thành công!", "success");
    } catch (err) {
      console.error(err);
      onShowNotification("Gặp sự cố khi gửi yêu cầu. Vui lòng liên hệ Hotline.", "error");
    } finally {
  ...
  const mapContent = useMemo(() => {
    ...
    if (project.mapHtml && (project.mapHtml.startsWith('<iframe') || project.mapHtml.includes('google.com/maps'))) {
      return (
        <div 
          ...
          dangerouslySetInnerHTML={{ __html: project.mapHtml.includes('iframe') ? project.mapHtml : `<iframe src="${project.mapHtml}"></iframe>` }}
        />
      );
    }
    if (project.location) {
    ...
    return (
      <div className="bg-slate-900 aspect-[21/9] rounded-xl border border-slate-800 flex flex-col items-center justify-center p-6 text-center space-y-2">
          ...
          <h4 className="text-white text-sm font-semibold">Chưa có bản đồ vị trí 1/500</h4>
          <p className="text-xs text-slate-500 max-w-md">Vị trí chi tiết trên bản đồ vệ tinh đang được xác minh, quý khách vui lòng liên hệ Hotline bên dưới để nhận tài liệu file sơ đồ dự án.</p>
      </div>
    );
  ...
  const renderCustomSections = (position: string) => {
    ...
    const sections = project.customSections.filter(s => s.position === position);
    if (sections.length === 0) return null;

    return sections.map((sec, idx) => (
      <div key={`cs-${position}-${sec.id || idx}`} className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pt-8 mt-[-32px]">
  ...
  if (loading) {
    return (
      <div className="py-32 text-center space-y-4 max-w-sm mx-auto" id="project-detail-loading">
        <div className="w-12 h-12 border-4 border-amber-500 border-t-transparent rounded-full animate-spin mx-auto" />
        <p className="text-slate-400 text-xs font-light">Đang mở tệp thông tin quy hoạch dự án...</p>
      </div>
    );
  }
  ...
  const galleryImages = project.imageUrls && project.imageUrls.length > 0
    ? project.imageUrls
    : [project.imageUrl || 'https://placehold.co/1200x600/1e293b/a4b5fd?text=No+Image', 'https://placehold.co/1200x600/0f172a/a4b5fd?text=Image+2', 'https://placehold.co/1200x600/334155/a4b5fd?text=Image+3', 'https://placehold.co/1200x600/475569/a4b5fd?text=Image+4', 'https://placehold.co/1200x600/64748b/a4b5fd?text=Image+5'];

  const rawBaseRating = project.baseRating || 5;
  const rawBaseCount = project.baseReviewCount || 0;
  const computedTotalStars = rawBaseRating * rawBaseCount + (project.userTotalRating || 0);
  ...
  return (
    <div className="pb-10" id="project-detail-view-root">
      <Helmet>
        ...
        <meta name="description" content={project.seoDesc || (project.description || "").replace(/<[^>]*>?/gm, '').substring(0, 160)} />
        {project.seoKeywords && <meta name="keywords" content={project.seoKeywords} />}
        <meta property="og:type" content="article" />
        <meta property="og:url" content={typeof window !== 'undefined' ? window.location.href : ''} />
        <meta property="og:title" content={project.seoTitle || project.title} />
        <meta property="og:description" content={project.seoDesc || (project.description || "").replace(/<[^>]*>?/gm, '').substring(0, 160)} />
      ...
      <div 
        ...
        onMouseLeave={() => setIsGalleryAutoplayPaused(false)}
      >
        ...
        <div className="relative w-full h-full flex items-center justify-center z-10">
          {galleryImages.map((img, idx) => {
            const offset = (idx - currentImageIndex + galleryImages.length) % galleryImages.length;
            ...
            if (isCenter) translation = 'translate-x-0 opacity-100 scale-100 z-30';
            else if (isPrev) translation = '-translate-x-[75vw] sm:-translate-x-[65vw] lg:-translate-x-[55vw] opacity-40 scale-90 z-20';
            else if (isNext) translation = 'translate-x-[75vw] sm:translate-x-[65vw] lg:translate-x-[55vw] opacity-40 scale-90 z-20';
            ...
            return (
              <div 
                ...
                onClick={() => !isCenter && setCurrentImageIndex(idx)}
              >
                <img
                  src={(img) || undefined}
        ...
        <div className="absolute top-4 sm:top-6 lg:top-8 left-0 right-0 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 z-40 pointer-events-none">
          <div className="flex items-center gap-2 text-[13px] md:text-sm text-slate-300 font-medium drop-shadow-md pointer-events-auto">
            ...
            <span className="cursor-pointer hover:text-white transition-colors" onClick={() => onNavigate({ screen: 'du-an' })}>Dự án</span>
            ...
            {galleryImages.map((img, idx) => (
              <button
                ...
                className={`relative w-16 sm:w-20 lg:w-24 aspect-[4/3] rounded-md overflow-hidden shrink-0 border-2 transition-all group/thumb ${
                  idx === currentImageIndex ? 'border-white shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'border-transparent hover:border-white/50 opacity-60 hover:opacity-100'
                }`}
              >
                <img src={(img) || undefined} alt={`Hình thu nhỏ tổng quan ${project.title} - ${idx + 1}`} className="w-full h-full object-cover" />
                {idx !== currentImageIndex && <div className="absolute inset-0 bg-black/30 group-hover/thumb:bg-black/10 transition-colors" />}
              </button>
            ))}
          </div>
      ...
      <div className={`sticky ${scrollDirection === 'down' ? 'top-0' : 'top-10'} z-40 bg-slate-950/95 backdrop-blur-md border-b border-t-0 border-slate-800 shadow-[0_10px_20px_rgba(0,0,0,0.5)] transition-all duration-300 w-full sm:mx-0 px-4 sm:px-0 h-[38px] pb-0`}>
        <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-8 flex items-center justify-between h-[30px] py-0">
          <div id="tab-navigation-container" className="flex items-center gap-2 sm:gap-6 overflow-x-auto hide-scrollbar h-[30px]">
            {TABS.map((tab) => {
      ...
      <div className="max-w-7xl mx-auto pl-[10px] pr-[10px] xl:px-8 pt-[10px] pb-[40px] lg:pt-[10px] lg:pb-[40px]">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 text-left">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div id="overview" ref={(el) => sectionRefs.current[0] = el} className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pt-8 mt-[-32px]">
              <h1 className="flex items-center gap-3 text-[25px] md:text-[26px] w-full max-w-[785px] font-bold font-serif my-[20px] drop-shadow-sm">
                ...
                <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Tổng quan dự án {project.title}</span>
              </h1>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl pr-[9px] md:pr-8 space-y-6 pt-[0px] pb-[10px] pl-[10px] md:pl-[32px] mb-[10px]">
                <h3 className="text-[15px] font-bold text-white mb-[5px] h-[37px] pb-[16px] pt-[9px] border-b border-slate-800">Thông tin chi tiết</h3>
                <div className="grid grid-cols-2 gap-x-4 sm:gap-x-12 gap-y-4 relative">
                  ...
                  {[
                    ...
                    { label: "Mật độ xây dựng", value: project.buildingDensity },
                    { label: "Thời gian bàn giao", value: project.handoverTime },
                    { label: "Phân khu", value: project.subdivisions },
                    { label: "Tiến độ", value: project.status === 'opening' ? 'Đang mở bán' : 'Đã bàn giao' },
                    { label: "Ngân hàng hỗ trợ", value: project.supportedBanks }
                    ...
                    return (
                    <div key={idx} className={`flex ${hasList ? 'flex-col items-start gap-1.5' : 'flex-col md:flex-row md:items-start md:justify-between gap-1 md:gap-4'} border-b border-slate-800/50 pb-[6px] md:pb-[2px]`}>
                      <span className="text-slate-400 text-[13px] md:text-sm whitespace-nowrap">{item.label}</span>
                      <div className={`text-white font-medium text-[13px] md:text-sm ${hasList ? 'text-left w-full grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1' : 'text-left md:text-right min-w-0 break-words'}`}>
                        {lines.map((line, i, arr) => {
                          const isListItem = line.trim().startsWith('+');
                          const content = isListItem ? line.trim().substring(1).trim() : line;
                          
                          const renderedContent = content.split(' ').map((word, j) => {
                            const urlRegex = /^(https?:\/\/[^\s]+)$/;
                            if (urlRegex.test(word)) {
                              return <a key={j} href={word} target="_blank" rel="noopener noreferrer" className="text-amber-500 hover:text-amber-400 underline">{word} </a>;
                            }
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: project.description || "<p>Chưa có thông tin tổng quan cập nhật bằng mã HTML.</p>" }}
                  />
                ...
                {!project.description && (
                    <div className="text-slate-400 text-[13px] md:text-sm italic border-l-2 border-amber-500 pl-4 py-2 bg-slate-900/50">
                      Nội dung tổng quan đang được cập nhật. Vui lòng quay lại sau.
                    </div>
                )}
              </div>
            ...
            {hasSubdivisionContent && (
              <div id="subdivision" ref={(el) => sectionRefs.current[7] = el} className="space-y-8 pt-[20px] mt-[-32px]">
                <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif mb-0 drop-shadow-sm">
                  ...
                  <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Khu của {project.title}</span>
                </h2>
                <div className="space-y-6">
                  {project.subdivisionTab && (
                    <div className="relative">
                      <div 
                        ...
                        dangerouslySetInnerHTML={{ __html: project.subdivisionTab }}
                      />
                  ...
                  {project.subdivisionTab && (
                    <button
                      onClick={() => setIsSubdivisionExpanded(!isSubdivisionExpanded)}
                      className="flex items-center gap-2 text-amber-500 hover:text-amber-400 font-medium text-[13px] md:text-sm transition-colors mt-2"
                    >
                      {isSubdivisionExpanded ? (
                        <>Thu gọn <ChevronUp className="w-4 h-4" /></>
                  ...
                  {project.subdivisionsCards && project.subdivisionsCards.length > 0 && (
                    <div className="flex overflow-x-auto gap-4 hide-scrollbar snap-x snap-mandatory pt-2 pb-6">
                       {project.subdivisionsCards.map((card, idx) => (
                         <div key={idx} className="min-w-[280px] w-[300px] shrink-0 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden snap-start flex flex-col items-start relative group">
                            <div className="relative w-full aspect-[16/9] overflow-hidden bg-slate-950">
                               ...
                               <img src={(card.imageUrl) || undefined} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt={card.name} referrerPolicy="no-referrer" />
                            </div>
                            <div className="px-5 py-[10px] w-full flex-grow flex flex-col justify-start">
                               <div className="space-y-3 mb-4">
                                 <div className="flex items-start justify-between gap-2 mb-[5px]">
                                   <div className="text-[13px] md:text-sm text-slate-400 shrink-0 mt-0.5">Phân khu</div>
                                   <div className="text-[13px] md:text-sm text-slate-200 font-medium text-right">{card.projectStr || project.title}</div>
                                 </div>
                                 <div className="flex items-start justify-between gap-2 pt-[10px] mb-0">
                                   <div className="text-[13px] md:text-sm text-slate-400 shrink-0 mt-0.5">Phong cách</div>
                                   <div className="text-[13px] md:text-sm text-slate-200 font-medium text-right">{card.styleStr || 'Hiện đại'}</div>
                                 </div>
                                 <div className="flex items-start justify-between gap-2">
                                   <div className="text-[13px] md:text-sm text-slate-400 shrink-0 mt-0.5">Giá bán</div>
                                   <div className="text-[13px] md:text-sm text-amber-500 font-bold text-right">{card.priceStr || 'Đang cập nhật'}</div>
                                 </div>
                               </div>
                               {card.types && card.types.length > 0 && (
                                 <div className="pt-3 mb-1 border-t border-slate-800">
                                    <div className="text-[11px] uppercase tracking-wider font-semibold text-slate-500 mb-2">Loại hình</div>
                                    <div className="grid grid-cols-2 gap-y-2 gap-x-2 text-[13px] text-slate-300">
                                       {card.types.map((type, tIdx) => (
                                         <div key={tIdx} className="flex items-center gap-1.5 truncate">
                               ...
                               <button 
                                 onClick={() => {
                                   if (card.linkedProjectId) {
                                     onNavigate({ screen: 'project-detail', projectId: card.linkedProjectId });
                                   } else {
            ...
            <div id="location" ref={(el) => sectionRefs.current[1] = el} className="space-y-8 pt-8 mt-[-32px]">
              <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                ...
                <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Vị trí {project.title}</span>
              </h2>
              <div className="space-y-6">
                ...
                {project.locationShortDesc && (
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: project.locationShortDesc }}
                  />
                )}
                ...
                <div className="relative">
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: project.locationTab || "<p>Chưa có bài viết giới thiệu vị trí cụ thể.</p>" }}
                  />
            ...
            <div id="amenity" ref={(el) => sectionRefs.current[2] = el} className="space-y-8 pt-8 mt-[-32px]">
              <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                <Sparkles className="w-[20px] h-[20px] md:w-8 md:h-8 text-amber-500 shrink-0" />
                <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Tiện ích {project.title}</span>
              </h2>
              <div className="space-y-6">
                <div className="relative">
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: project.amenityTab || `<p>${project.title} sở hữu hệ sinh thái tiện ích đẳng cấp, đáp ứng trọn vẹn mọi nhu cầu sống, học tập, làm việc và giải trí.</p>` }}
                  />
                ...
                {project.amenityImages && project.amenityImages.length > 0 && (
                  <div className="pt-4 space-y-4">
                    <div 
                      ...
                      onMouseLeave={() => setIsAmenityAutoplayPaused(false)}
                    >
                      <div className="absolute inset-0 flex items-center justify-center">
                        <img 
                          src={(project.amenityImages[currentAmenityImageIndex]) || undefined} 
                      ...
                      {project.amenityImages.length > 1 && (
                        <>
                          <button 
                            ...
                            onClick={() => setCurrentAmenityImageIndex(prev => prev === 0 ? project.amenityImages!.length - 1 : prev - 1)}
                          >
                             <ChevronLeft className="w-5 h-5" />
                          </button>
                          <button 
                            ...
                            onClick={() => setCurrentAmenityImageIndex(prev => prev === project.amenityImages!.length - 1 ? 0 : prev + 1)}
                          >
                    ...
                    {project.amenityImages.length > 1 && (
                      <div 
                        className="flex gap-2 overflow-x-auto hide-scrollbar snap-x pb-2"
                        onMouseEnter={() => setIsAmenityAutoplayPaused(true)}
                        onMouseLeave={() => setIsAmenityAutoplayPaused(false)}
                      >
                        {project.amenityImages.map((img, idx) => (
                          <button
                            key={idx}
                            onClick={() => setCurrentAmenityImageIndex(idx)}
                            className={`relative w-20 sm:w-24 aspect-video rounded-md overflow-hidden shrink-0 border-2 transition-all snap-start ${
                              idx === currentAmenityImageIndex ? 'border-amber-500 shadow-sm' : 'border-slate-800 hover:border-slate-600 opacity-60 hover:opacity-100'
                            }`}
                          >
                            <img src={(img) || undefined} alt={`Hình thu nhỏ tiện ích ${project.title} - ${idx + 1}`} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                          </button>
                        ))}
                      </div>
                    )}
            ...
            <div id="floor-plan" ref={(el) => sectionRefs.current[3] = el} className="space-y-8 pt-8 mt-[-32px]">
              <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                <LayoutGrid className="w-[20px] h-[20px] md:w-8 md:h-8 text-amber-500 shrink-0" />
                <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Mặt bằng {project.title}</span>
              </h2>
              ...
              {project.floorPlanImages && project.floorPlanImages.length > 0 && (
                <div 
                  ...
                  onMouseLeave={() => setIsFloorPlanAutoplayPaused(false)}
                >
                  <div className="absolute inset-0 flex items-center justify-center">
                    <img 
                      src={(project.floorPlanImages[currentFloorPlanImageIndex]) || undefined} 
                  ...
                  {project.floorPlanImages.length > 1 && (
                    <>
                      <button 
                        className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 hover:bg-black/70 text-white rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10 opacity-0 group-hover:opacity-100 transition-all pointer-events-auto z-10"
                        onClick={() => setCurrentFloorPlanImageIndex(prev => prev === 0 ? project.floorPlanImages!.length - 1 : prev - 1)}
                      >
                         <ChevronLeft className="w-5 h-5" />
                      </button>
                      <button 
                        className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 hover:bg-black/70 text-white rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10 opacity-0 group-hover:opacity-100 transition-all pointer-events-auto z-10"
                        onClick={() => setCurrentFloorPlanImageIndex(prev => prev === project.floorPlanImages!.length - 1 ? 0 : prev + 1)}
                      >
                         <ChevronRight className="w-5 h-5" />
                      </button>
                      ...
                      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-10">
                        {project.floorPlanImages.map((_, idx) => (
                          <button
                            key={idx}
                            onClick={() => setCurrentFloorPlanImageIndex(idx)}
                ...
                <div className="pt-6">
                  ...
                  <div className="mt-6">
                    {project.floorPlanTabs.map((tab) => (
                      <div 
                        key={tab.id} 
                        style={{ display: currentFloorPlanTabId === tab.id ? 'block' : 'none' }}
                        ...
                        {tab.images && tab.images.length > 0 && (
                          <div className="pt-4 space-y-4">
                            <div className="relative w-full aspect-video md:aspect-[21/9] bg-slate-950 border border-slate-800 rounded-xl overflow-hidden group">
                              <div className="absolute inset-0 flex items-center justify-center">
                                <img 
                                  src={(tab.images[currentFloorPlanImageIndex] || tab.images[0]) || undefined} 
                              ...
                              {tab.images.length > 1 && (
                                <>
                                  <button 
                                    className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 hover:bg-black/70 text-white rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10 opacity-0 group-hover:opacity-100 transition-all pointer-events-auto z-10"
                                    onClick={() => setCurrentFloorPlanImageIndex(prev => prev === 0 ? tab.images!.length - 1 : prev - 1)}
                                  >
                                    <ChevronLeft className="w-5 h-5" />
                                  </button>
                                  <button 
                                    className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 hover:bg-black/70 text-white rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10 opacity-0 group-hover:opacity-100 transition-all pointer-events-auto z-10"
                                    onClick={() => setCurrentFloorPlanImageIndex(prev => prev === tab.images!.length - 1 ? 0 : prev + 1)}
                                  >
                                    <ChevronRight className="w-5 h-5" />
                                  </button>
                                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-full border border-white/10">
                                    {currentFloorPlanImageIndex + 1} / {tab.images.length}
                                  </div>
                                </>
                              )}
                            </div>
                              ...
                              <div className="flex gap-2 overflow-x-auto hide-scrollbar snap-x pb-2">
                                {tab.images.map((img, idx) => (
                                  <button
                                    ...
                                    onClick={() => setCurrentFloorPlanImageIndex(idx)}
                                    className={`relative w-20 sm:w-24 aspect-video rounded-md overflow-hidden shrink-0 border-2 transition-all snap-start ${
                                      idx === currentFloorPlanImageIndex ? 'border-amber-500 shadow-sm' : 'border-slate-800 hover:border-slate-600 opacity-60 hover:opacity-100'
                                    }`}
                                  >
                                    <img src={(img) || undefined} alt={`Hình thu nhỏ mặt bằng ${tab.name} - ${idx + 1}`} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                  </button>
                                ))}
                              </div>
                            )}
              ...
              {!project.floorPlanTab && (!project.floorPlanTabs || project.floorPlanTabs.length === 0) && (
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center space-y-4 shadow-sm">
                  ...
                  <p className="text-xl font-bold text-amber-400">Đang cập nhật mặt bằng</p>
                  <p className="text-[13px] md:text-sm text-slate-400 max-w-lg mx-auto">Vui lòng đăng ký nhận tư vấn chuyên sâu để nhận thông tin sơ đồ mặt bằng và thiết kế chi tiết.</p>
                </div>
              )}
            </div>
            ...
            <div id="price" ref={(el) => sectionRefs.current[4] = el} className="space-y-8 pt-8 mt-[-32px]">
              <div className="flex justify-between items-end mb-4">
                <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                  <Banknote className="w-[20px] h-[20px] md:w-8 md:h-8 text-amber-500 shrink-0" />
                  <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Giá bán {project.title}</span>
                </h2>
                {targetProductCategory ? (
                  <button onClick={() => onNavigate({ screen: 'category-product', categoryName: targetProductCategory })} className="text-amber-500 hover:text-amber-400 text-[13px] md:text-sm font-medium whitespace-nowrap">Xem thêm</button>
                ) : (
                  <button onClick={() => onNavigate({ screen: 'san-pham' })} className="text-amber-500 hover:text-amber-400 text-[13px] md:text-sm font-medium whitespace-nowrap">Xem thêm</button>
                )}
              </div>
              {project.priceTab ? (
                <div className="space-y-6">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center space-y-4 shadow-sm">
                  ...
                  <p className="text-xl font-bold text-amber-400">Đang cập nhật chính sách giá</p>
                  <p className="text-[13px] md:text-sm text-slate-400 max-w-lg mx-auto">Vui lòng đăng ký nhận tư vấn chuyên sâu để nhận thông tin bảng giá chi tiết.</p>
                </div>
              )}
              ...
              {relatedProducts.length > 0 && (
                <div className="pt-8">
                  <h3 className="text-xl font-bold font-serif text-white mb-6">Sản phẩm thuộc dự án</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {relatedProducts.map((prod) => (
                      <ProductCard key={prod.id} item={prod} onNavigate={onNavigate} />
                    ))}
                  </div>
                </div>
              )}
            ...
            {relatedProjects.length > 0 && (
              <div className="space-y-8 pt-12 mt-[-32px] border-t border-slate-800/50">
                <div className="flex justify-between items-end mb-4">
                  <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                    <Building2 className="w-[20px] h-[20px] md:w-8 md:h-8 text-amber-500 shrink-0" />
                    <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Dự án liên quan</span>
                  </h2>
                  <button onClick={() => onNavigate({ screen: 'du-an' })} className="text-amber-500 hover:text-amber-400 text-[13px] md:text-sm font-medium whitespace-nowrap">Xem thêm</button>
                </div>
                  ...
                  {relatedProjects.map((proj) => (
                    ...
                    >
                      <div className="relative aspect-[4/3] overflow-hidden">
                        <img src={(proj.imageUrl) || undefined} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={proj.title} onError={(e) => (e.target as HTMLImageElement).src = 'https://placehold.co/600x400/1e293b/a4b5fd?text=No+Image'} />
                        <div className="absolute top-2 left-2 px-2 py-1 bg-amber-500 text-slate-950 text-[10px] font-bold uppercase rounded">Đang mở bán</div>
                      </div>
                      <div className="p-4 flex-1 flex flex-col">
                         <h4 className="text-lg font-bold text-white mb-2 line-clamp-1">{proj.title}</h4>
                         <div className="flex items-center gap-1 text-slate-400 text-xs mb-3">
                           <MapPin className="w-3.5 h-3.5" />
                           <span className="truncate">{proj.locationTab?.replace(/<[^>]*>?/gm, '').substring(0, 30) || proj.title}</span>
                         </div>
                         <div className="grid grid-cols-2 gap-2 mt-auto pt-3 border-t border-slate-800 text-[11px] text-slate-300">
                           <div className="flex items-center gap-1.5"><Building2 className="w-3 h-3 text-amber-500"/> {proj.developer || 'Đang cập nhật'}</div>
                           <div className="flex items-center gap-1.5"><Leaf className="w-3 h-3 text-emerald-500"/> {proj.scale || 'Đang cập nhật'}</div>
                         </div>
                      </div>
                    </div>
                  ))}
            ...
            <div id="qa" ref={(el) => sectionRefs.current[8] = el} className="space-y-8 pt-12 mt-[-32px] border-t border-slate-800/50">
              <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                <HelpCircle className="w-[20px] h-[20px] md:w-8 md:h-8 text-amber-500 shrink-0" />
                <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Hỏi đáp {project.title}</span>
              </h2>
              ...
              <div className="space-y-6">
                {project.qaList && project.qaList.length > 0 ? (
                  <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                    {project.qaList.map((qa, index) => {
                      ...
                      return (
                        ...
                        >
                          <button
                            onClick={() => setActiveQaIndex(isActive ? null : index)}
                  ...
                  <p className="text-slate-400 italic">Chưa có thông tin hỏi đáp cho dự án này.</p>
                )}
                
                {(!project.qaList || project.qaList.length === 0) && project.qaTab && (
                  <button 
            ...
            <div id="news" ref={(el) => sectionRefs.current[5] = el} className="space-y-8 pt-12 mt-[-32px] border-t border-slate-800/50">
              <div className="flex justify-between items-end mb-4">
                <h2 className="flex items-center gap-3 text-[20px] md:text-[26px] font-bold font-serif drop-shadow-sm">
                  <Newspaper className="w-[20px] h-[20px] md:w-8 md:h-8 text-amber-500 shrink-0" />
                  <span className="text-[20px] md:text-[26px] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">Tin tức dự án</span>
                </h2>
                {targetNewsCategory ? (
                  <button onClick={() => onNavigate({ screen: 'category-news', categoryName: targetNewsCategory })} className="text-amber-500 hover:text-amber-400 text-[13px] md:text-sm font-medium whitespace-nowrap">Xem thêm</button>
                ) : (
                  <button onClick={() => onNavigate({ screen: 'tin-tuc' })} className="text-amber-500 hover:text-amber-400 text-[13px] md:text-sm font-medium whitespace-nowrap">Xem thêm</button>
                )}
              </div>
              <div className="flex overflow-x-auto gap-6 snap-x snap-mandatory pb-4 hide-scrollbar">
                {relatedNews.length > 0 ? relatedNews.map((news) => (
                  <div 
                    key={news.id} 
                    onClick={() => onNavigate({ screen: 'news-detail', newsId: news.id })} 
                  ...
                  >
                    <div className="relative h-48 overflow-hidden">
                      <img src={(news.imageUrl) || undefined} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={news.title} onError={(e) => (e.target as HTMLImageElement).src = 'https://placehold.co/600x400/1e293b/a4b5fd?text=No+Image'} />
                    </div>
                    <div className="px-5 py-2.5 flex-1 flex flex-col">
                       <h4 className="text-base font-bold text-white line-clamp-2 group-hover:text-amber-400 transition-colors mb-2">{news.title}</h4>
                       <span className="text-xs text-slate-400 font-medium">{news.createdAt ? new Date(news.createdAt).toLocaleDateString('vi-VN') : ''}</span>
                       <div className="w-full h-px bg-slate-800/60 mt-0 mb-3"></div>
                       <p className="text-[13px] font-light md:text-[13px] font-light md:text-[13px] font-light md:text-[13px] font-light md:text-[13px] font-light md:text-[13px] font-light md:text-[13px] font-light md:text-[13px] font-light md:text-sm text-slate-300 line-clamp-3 mb-4 flex-1">{news.description}</p>
            ...
            <div id="contact" ref={(el) => sectionRefs.current[6] = el} className="absolute h-0 w-full" aria-hidden="true"></div>
          ...
          {/* Right Column (Form) */}
          <div className="lg:col-span-4 space-y-6 relative" id="project-detail-sidebar">
            <div className={`sticky ${scrollDirection === 'down' ? 'top-[48px]' : 'top-[96px] md:top-[100px]'}`}>
              ...
              <div className="bg-slate-900 border border-slate-800 px-[20px] py-[10px] rounded-xl space-y-5 shadow-xl relative text-left">
                <div className="text-center space-y-1 pb-[5px] border-b border-slate-800/50 mb-[5px]">
                  ...
                  <p className="text-xs text-slate-400 pb-[5px]">Chuyên viên Greenia Homes hỗ trợ 24/7</p>
                </div>
                ...
                <div className="space-y-3 mb-[10px]">
                  <div className="flex items-start gap-2.5 pt-[5px] mb-[5px]">
                    ...
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      <span className="font-semibold text-white">Phân tích</span> quỹ căn, chính sách, tiện ích giúp Khách hàng lựa chọn căn <span className="font-semibold text-amber-400">tốt nhất.</span>
                    </p>
                  </div>
                  <div className="flex items-start gap-2.5 mb-[5px]">
                    ...
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      <span className="font-semibold text-white">Giải đáp mọi thắc mắc</span> của khách hàng nhanh chóng.
                    </p>
                  </div>
                  <div className="flex items-start gap-2.5">
                    ...
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      <span className="font-semibold text-white">Tuyệt đối bảo mật</span> thông tin cá nhân.
                    </p>
                  </div>
                </div>
                ...
                {formSubmitted ? (
                  <div className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-xl p-5 text-center space-y-3 animate-in zoom-in-95">
                    ...
                    <div>
                      <h5 className="font-medium text-[15px] text-white">Đăng ký thành công!</h5>
                  ...
                  <form onSubmit={handleConsultSubmit} className="space-y-3.5 pt-[10px]">
                    ...
                    <div className="space-y-2.5 pt-2">
                      <label className="flex items-start gap-2.5 cursor-pointer group">
                        ...
                        <span className="text-[10px] text-slate-400 leading-snug group-hover:text-slate-300">
                          Tôi đã đọc và đồng ý với <button type="button" onClick={() => onNavigate({ screen: 'terms-of-use' })} className="underline text-amber-500 hover:text-amber-400">Điều khoản & Điều kiện</button> của Greenia Market.
                        </span>
                      </label>
                      <label className="flex items-start gap-2.5 cursor-pointer group">
                        <input
                        ...
                        />
                        <span className="text-[10px] text-slate-400 leading-snug group-hover:text-slate-300">
                          Tôi đã đọc và đồng ý với <button type="button" onClick={() => onNavigate({ screen: 'privacy-policy' })} className="underline text-amber-500 hover:text-amber-400">Chính sách bảo mật dữ liệu cá nhân</button> của Greenia Market.
                        </span>
                      </label>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2">
                       <a href="tel:0932966700" className="flex flex-col items-center justify-center gap-1 bg-slate-950/40 hover:bg-slate-800 border border-slate-800 text-slate-300 rounded-lg py-2 transition-colors cursor-pointer text-center">
                         <Phone className="w-3.5 h-3.5" />
                         <span className="text-[10px] font-medium">Gọi trực tiếp</span>
                       </a>
                       <a className="flex flex-col items-center justify-center gap-1 bg-slate-950/40 hover:bg-slate-800 border border-slate-800 text-slate-300 rounded-lg py-2 transition-colors cursor-pointer text-center">
                         <i className="fab fa-diaspora text-[14px]"></i>
