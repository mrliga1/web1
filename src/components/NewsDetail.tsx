...
export default function NewsDetail({ newsId, onNavigate, onShowNotification }: NewsDetailProps) {
  ...
  const handleSidebarConsultSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!clientName.trim() || !clientPhone.trim() || !clientDemand.trim()) {
  ...
  // Featured 3 products for Sidebar Column 2 (viewsCount desc)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-[10px] space-y-12 animate-in fade-in" id="news-detail-root-container">
      <Helmet>
      ...
      <div>
        ...
        <div className="text-left max-w-4xl" id="news-detail-briefing">
          ...
          <h1 className="mt-[20px] mb-[10px] text-2xl sm:text-3.5xl font-display font-medium text-white tracking-tight leading-snug">
            {article.title}
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="bg-slate-900/40 border border-slate-900 p-6 rounded-lg space-y-4 overflow-hidden" id="article-related-links">
            ...
            <div className="relative overflow-x-auto pb-4 scrollbar-thin snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0">
              <div className="flex gap-4 box-border">
                {generalNewsList.filter(n => n.id !== article.id && n.category === article.category).slice(0, 6).map((n) => (
                  <div
                    key={n.id}
                    onClick={() => onNavigate({ screen: 'news-detail', newsId: n.id })}
                    className="w-full lg:w-[calc(33.333%-10.66px)] shrink-0 bg-slate-900/30 border border-slate-850 hover:border-amber-555 rounded-lg p-3.5 space-y-3 cursor-pointer transition-all snap-start"
                  >
        ...
        <div className="lg:col-span-4 space-y-8" id="news-detail-sidebar">
          
          {/* Yêu cầu tư vấn Form widget */}
          <div className="bg-slate-900 border border-slate-850 p-6 rounded-lg space-y-4 shadow-xl">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider pb-3 border-b border-slate-800">
            ...
            {formSubmitted ? (
              ...
              <form onSubmit={handleSidebarConsultSubmit} className="space-y-4">
          ...
          <div className="bg-slate-900 border border-slate-850 p-[10px] mb-[20px] rounded-lg space-y-4">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider pb-0 border-b border-slate-800">
              Danh Mục Sản Phẩm
            </h4>
          ...
          <div className="bg-slate-900/40 border border-slate-900 p-[10px] mb-[20px] rounded-lg space-y-4">
            <div className="border-b border-slate-900 pb-0 mb-[10px] flex items-center justify-between">
              <h4 className="text-white text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="space-y-4">
              {sidebarFeaturedProps.map((p) => (
                <div
                  key={p.id}
                  onClick={() => onNavigate({ screen: 'product-detail', productId: p.id })}
          ...
          {/* Newest News titles */}
          <div className="bg-slate-900 border border-slate-850 p-[10px] rounded-lg space-y-4">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider pb-0 mb-[10px] border-b border-slate-800">
            ...
            <div className="space-y-3">
              {sidebarNewestNews.map((n) => (
                <div
                  key={n.id}
                  onClick={() => onNavigate({ screen: 'news-detail', newsId: n.id })}
                  className="text-left group cursor-pointer border-b border-slate-950 pb-2 last:border-0"
                >
                  <h5 className="text-[11px] font-semibold text-slate-300 group-hover:text-amber-400 leading-snug line-clamp-2">
                    {n.title}
                  </h5>
      ...
      {/* Under columns: Same category articles slider in 5 columns with See More returning back */}
      <section className="space-y-4 pt-8 border-t border-slate-900 overflow-hidden" id="same-category-slide-section">
        <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-end gap-3">
          <div>
          ...
          <button
            ...
            className="inline-flex items-center gap-1 bg-slate-900 border border-slate-800 text-xs font-semibold text-amber-400 px-4 py-2 rounded-full cursor-pointer transition-all shrink-0"
          >
            <span>Về trang chủ chuyên mục tin</span>
        ...
        <div className="relative overflow-x-auto pb-4 scrollbar-thin snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0">
          <div className="flex gap-5 box-border">
            {generalNewsList.filter(n => n.id !== article.id && n.category === article.category).slice(0, 5).map((n) => (
              <div
                key={n.id}
                onClick={() => onNavigate({ screen: 'news-detail', newsId: n.id })}
                className="w-full sm:w-[280px] lg:w-[calc(20%-16px)] shrink-0 bg-slate-900/30 border border-slate-850 hover:border-amber-555 rounded-lg p-3.5 space-y-3 cursor-pointer transition-all snap-start"
              >
                <img src={(n.imageUrl) || undefined} alt={n.title} className="w-full h-40 sm:h-32 lg:h-24 object-cover rounded-lg" referrerPolicy="no-referrer" />
                <div className="text-left space-y-1">
                  <h4 className="text-sm lg:text-xs font-semibold text-white line-clamp-2">{n.title}</h4>
                  <span className="text-[10px] lg:text-[9px] text-slate-500 font-mono block mt-1">
                    {new Date(n.createdAt).toLocaleDateString('vi-VN')}
