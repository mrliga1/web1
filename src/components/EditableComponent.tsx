...
export function EditableText({
  ...
  if (!isEditMode) {
    ...
    const lines = val.split('\n').map((line, lidx) => {
      ...
      return <React.Fragment key={lidx}>{line}{lidx < val.split('\n').length - 1 ? <br /> : null}</React.Fragment>;
    });
...
export function EditableImage({
    ...
    try {
      ...
      try {
        ...
        if (result && result.url) {
          ...
          onShowNotification('Đã tải hình ảnh trực tiếp lên Cloud lưu trữ!', 'success');
        } else {
          setUrl(base64);
          updateImage(base64);
  ...
  return (
    <div className={`relative group/img ${className}`}>
      {url ? (
        <img src={(url) || undefined} alt="Bố cục" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
      ) : (
