        ...
        <div className="flex items-center justify-between h-10 md:h-10 relative">
          ...
          <div className="hidden sm:flex items-center gap-2" id="user-controls">
            ...
            {currentUser ? (
              ...
              >
                {/* Direct Quản trị Button next to avatar */}
                <button
                  onClick={() => onNavigate({ screen: 'admin' })}
                  className={`border py-1 px-2.5 rounded-full text-[10.5px] font-semibold transition-all cursor-pointer flex items-center gap-1 shrink-0 ${
                    theme === 'dark'
                      ? 'bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border-amber-500/20'
                      : 'bg-[#eef3f6] hover:bg-slate-200/50 text-slate-800 border-slate-200/50'
                  }`}
                >
                ...
                {/* Micro avatar with sliding expansion hover effect */}
                <div className="relative group">
                  <button
                    id="user-menu-btn"
                    className={`flex items-center gap-0 border p-0.5 rounded-full transition-all text-[10.5px] font-semibold cursor-pointer select-none ${
                      theme === 'dark'
                        ? 'bg-slate-900 border-slate-850 group-hover:border-amber-500/30'
                  ...
                  >
                    <div className="shrink-0">
                      {currentUser.photoURL ? (
                        ...
                        <div className={`w-4.5 h-4.5 rounded-full flex items-center justify-center ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`}>
                          <UserIcon className="w-2.5 h-2.5 text-amber-450" />
                        </div>
                      )}
                    </div>
                    
                    {/* Collapsed text that expands smoothly on hover */}
                    <div className="max-w-0 overflow-hidden group-hover:max-w-[130px] group-hover:px-1.5 transition-all duration-300 ease-out flex items-center gap-1">
                      <span className={`max-w-[60px] truncate ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>
                        {currentUser.displayName || currentUser.email?.split('@')[0]}
                      </span>
                      <span className={`text-[6.5px] px-0.5 py-0.5 rounded shrink-0 capitalize ${theme === 'dark' ? 'bg-slate-950 text-slate-400' : 'bg-slate-250 text-slate-600'}`}>
                        {isAdmin ? 'Admin' : 'Mbr'}
                      </span>
                    </div>
                  </button>
                  ...
                  {userDropdownOpen && (
                    <div 
                      id="dropdown-user"
                      className={`absolute right-0 mt-1 w-36 border rounded-lg shadow-2xl z-20 py-0.5 overflow-hidden text-left ${
                        theme === 'dark'
          ...
          <div className="flex lg:hidden items-center gap-2" id="mobile-menu-controls">
            ...
            <a 
              href="tel:0932966700" 
              className="hidden md:flex items-center justify-center bg-amber-500 text-slate-950 p-2.5 rounded-full shrink-0 animate-pulse"
              title="Gọi Ngay"
            >
            ...
            <button
              onClick={() => onNavigate({ screen: 'favorites' })}
              className="md:hidden flex items-center justify-center border border-amber-500/20 text-amber-500 hover:bg-amber-500/20 p-2 rounded-lg"
              title="Yêu thích"
            >
